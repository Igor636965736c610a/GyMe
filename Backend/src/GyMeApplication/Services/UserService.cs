...
internal class UserService : IUserService
{
    ...
    public UserService(IUserRepo userRepo, IUserContextService userContextService, IGyMeMapper gyMeMapper)
    {
        _userRepo = userRepo;
    ...
    public async Task<IEnumerable<GetUserDto>> FindUsers(string key, int page, int size)
    {
        ...
        var friendsStatus = await _userRepo.GetFriends(userIdFromJwt, users.Select(x => x.Id));

        return users.Select(x => _gyMeMapper.GetUserDtoMap(x, GetValueOrDefault(friendsStatus, x.Id)));
    }
    ...
    public async Task<IEnumerable<GetUserDto>> GetFriends(FriendStatusDto friendStatusDto, int page, int size)
    {
        ...
        var friendsDto = friends.Select(x => _gyMeMapper.GetUserDtoMap(x.Friend, x.FriendStatus));
    ...
    public async Task<IEnumerable<CommonFriendsResultDto>> GetCommonFriends(int page, int size)
    {
        ...
        var commonFriendsDto = commonFriends.Select(x => _gyMeMapper.GetCommonFriendsResultDtoMap(x));
    ...
    private FriendStatus? GetValueOrDefault(Dictionary<Guid, FriendStatus> dictionary, Guid key)
    {
        return dictionary.TryGetValue(key, out FriendStatus value) ? value : null;
    }
}