...
internal class UserRepo : IUserRepo
{
    ...
    public async Task<Dictionary<Guid, FriendStatus>> GetFriends(Guid userId, IEnumerable<Guid> friendsId)
        => await _gyMePostgresContext.UserFriends
            .Where(x => x.UserId == userId && friendsId.Contains(x.FriendId))
            .ToDictionaryAsync(x => x.FriendId, x => x.FriendStatus);
    ...
    public async Task<IEnumerable<CommonFriendsResult>> GetCommonFriendsSortedByCount(Guid userId, int page, int size = 20)
        => await _gyMePostgresContext.UserFriends
            .Where(x => x.UserId == userId && x.FriendStatus == FriendStatus.Friend)
            .SelectMany(x => x.Friend.Friends)
            .Where(x => x.FriendId != userId && x.FriendStatus == FriendStatus.Friend && x.FriendStatus != FriendStatus.Blocked)
            .Include(x => x.Friend)
            .ThenInclude(x => x.ExtendedUser)
            ...
            {
                User = group.First().Friend,
                UserFriend = group.SelectMany(x => x.Friend.Friends).FirstOrDefault(x => x.UserId == userId && x.FriendId == group.Key.Id),
